...
/**
 * Differencing class for Poodll Paper
 *
 * A class containing functions for computing diffs between submitted text and corrected text
 ...
 */
    ...
    public static function fetchSequences($passage, $transcript, $alternatives, $language, $transcriptphones = false, $passagephones = false)
    {
        ...
        for ($pstart = 0; $pstart < $p_length; $pstart++) {
            ...
            while (
                ...
                // We don't have phoneme info in mod_paper.
                $tphoneword = false;
                $pphoneword = false;
                ...
                if (!$match && $alternatives) {
                    $alt_result = self::check_alternatives_for_match(
                        $passageword,
                        $transcriptword,
                        $alternatives
                    );
    ...
    public static function fetchDiffs($sequences, $passagelength, $transcriptlength, $debug = false)
    {
        ...
        usort($sequences, array('\mod_paper\diff','cmp'));
        ...
        foreach ($sequences as $sequence) {
            ...
            if (!$bust) {
                foreach ($priorsequences as $priorsequence) {
                    ...
                    if (
                        $sequence->tposition >= $priorsequence->tposition &&
            ...
            if (!$bust) {
                ...
                if (($altratio >= 0.5) && $enddistance > 0) {
                    $bust = true;
                } elseif ($sequence->length < $enddistance && $sequence->length < 3) {
                    $bust = true;
                }
            }
